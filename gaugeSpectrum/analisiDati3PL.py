    ...
    for metodo in range(len(metodi)):
        ...
        for i in range(nMisCampione):
            ...
            if esperimento == 2 or esperimento == 6: #degeneri con quello prima
                errSov -= abs(np.vdot(autovettori[esperimento - 1], autovettoriStimati[i])) ** 2
        ...
        plt.yticks(ticks = [])
        plt.errorbar(medie, indiciMisure, xerr = errori, fmt = "o", label = "Misure", markersize = 10, linewidth = 2)
        plt.errorbar([mediaMisurata], [len(medie) + 1], xerr = [erroreMisurato], fmt = "^", label = "Stima del valore atteso", markersize = 10, linewidth = 2)
