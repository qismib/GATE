...
parametriGaussiane = [  [ 5.94447581,  0.12128139, 26.56598046],
...
for s in range(1, 2):
    ...
    for T in durateEvT:
        ...
        ordinateProbPrecise = [[] for n in range(nMis)]
        ...
        percorsoFile = "data/errori/"
        if not leggiMisureDaFile:
            #### Simulazione esatta
            
            for j in range(tentativi):
                ordinateFlutt = [[] for n in range(nMis)]
                ...
                moduli.scriviMisureSuFile(ordinateFlutt, percorsoFile + "EvTemp" + str(j))
                probAggregata.append(ordinateFlutt[0])
                
        
        else:
            for j in range(tentativi):
                ordinateFlutt = moduli.leggiMisureDaFile(percorsoFile + "EvTemp" + str(j), nMis, nPuntiEsatta)
                probAggregata.append(ordinateFlutt[0])
        ...
        suffissoFigure = "_" + str(T) + "_" + str(s) + "_" + str(shots) + "_shots" + ".png"
        ...
        plt.errorbar(x = ascisseEvEsatta, y = medieProb, yerr = erroriProb, marker = "o", color = "b", linestyle = "")
        ...
        parametri, covarianza = curve_fit(gaussiana, xDaFittare, yDaFittare, p0 = [6, 1, 10], sigma = erroriDaFittare)
       
        nXPrecise = 500
        xPrecise = [min(xDaFittare) + ((max(xDaFittare) - min(xDaFittare)) * x / nXPrecise) for x in range(nXPrecise)]
        valoriGaussiana = [gaussiana(x, parametri[0], parametri[1], parametri[2]) for x in xPrecise]
        ...
        plt.errorbar(x = ascisseTF, y = trFourier, yerr = erroriTrFourier, marker = ".", color = "b", linestyle = "--")
        ...
        plt.vlines(x = [ascisseTF[indiceFreqMax], ascisseTF[indiceFreqMax - 1], ascisseTF[indiceFreqMax + 1], 5.891], 
                   ymin = 0, ymax = [np.max(trFourier)], colors = ["r", "g", "g", "tab:olive"])
