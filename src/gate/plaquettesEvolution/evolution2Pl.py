...
for s in range(1, nStati):
    coefficienti = [0 for _ in range(nStati)]
    ...
    if leggiDaFile:
        ...
        probStati = [[] for _ in range(nStati)]
        for n in range(int(minP * nPunti), nPunti):
            ...
            res = (
                backend.run(circuitoTradotto, shots=ripetizioni).result().get_counts(qc)
            )
    ...
    ordinateProb = [[] for _ in range(len(H))]
    for t in ascisseEvEsatta:
        stato = moduli.evoluzioneHamiltoniana(
            vIniziale, autovalori, matriceAutovettori, matriceAutovettoriInversa, t
        )
